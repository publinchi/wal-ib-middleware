package com.cobiscorp.ecobis.orchestration.core.ib.query.stocks;
 ...
 * 
 */
@Component(name = "StockFactorOrchestrationCore", immediate = false)
@Service(value = { ICISSPBaseOrchestration.class, IOrchestrator.class })
@Properties(value = { @Property(name = "service.description", value = "StockFactorOrchestrationCore"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "4.6.1.0"),
		@Property(name = "service.identifier", value = "StockFactorOrchestrationCore") })
public class StockFactorOrchestrationCore extends SPJavaOrchestrationBase {

	private static ILogger logger = LogFactory.getLogger(StockFactorOrchestrationCore.class);
	private static final String CLASS_NAME = "StockFactorOrchestrationCore--->";
	 ...
	 */
	@Reference(referenceInterface = ICoreServiceStock.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, bind = "bindCoreServiceStock", unbind = "unbindCoreServiceStock")
	protected ICoreServiceStock coreServiceStock;
	 ...
	 * @param service
	 */
	public void bindCoreServiceStock(ICoreServiceStock service) {
		coreServiceStock = service;
	}
	 ...
	 * @param service
	 */
	public void unbindCoreServiceStock(ICoreServiceStock service) {
		coreServiceStock = null;
	}
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOriginalRequest,
		...
		try {
			...
			mapInterfaces.put("coreServiceStock", coreServiceStock);
	...
	private IProcedureResponse executeSteps(Map<String, Object> aBag) {
		...
		try {
			if (anOriginalRequest.readValueParam("@i_operacion").equals("S")) // Search
			{
				// consulta de stocks
	...
	private IProcedureResponse queryProviderTransaction(IProcedureRequest anOriginalRequest, Map<String, Object> aBag) {
		...
		try {
			...
			wStockResp = coreServiceStock.getStock(wStockRequest);
	...
	private StockRequest transformStockRequest(IProcedureRequest aRequest) {
		 ...
		 * 
		 * messageError = aRequest.readValueParam("@i_region") == null ?
		 * " - @i_region can't be null":""; messageError +=
		 * aRequest.readValueParam("@i_oficina") == null ?
		 * " - @i_oficina can't be null":""; messageError +=
		 * aRequest.readValueParam("@i_secuencial") == null ?
		 * " - @i_secuencial can't be null":"";
		 * 
		 * 
		 * if (!messageError.equals("")) throw new
	...
	private IProcedureResponse transformStockResponse(StockResponse aStockResponse,
		...
		if (aStockResponse.getReturnCode() != 0) {
			...
			metaData.addColumnMetaData(new ResultSetHeaderColumn("TARJETAS NO ASIGNADAS", ICTSTypes.SYBINT4, 5));
			metaData.addColumnMetaData(new ResultSetHeaderColumn("TARJETAS ASIGNADAS", ICTSTypes.SYBINT4, 5));
			...
			for (Stock aStock : aStockResponse.getStockCollection()) {
				...
				row.addRowData(6, new ResultSetRowColumnData(false, aStock.getNo_assigned().toString()));
				row.addRowData(7, new ResultSetRowColumnData(false, aStock.getAssigned().toString()));
			...
			wProcedureResponse.addParam("@o_stock", ICTSTypes.SYBINT4, 0,
					String.valueOf(aStockResponse.getTotalStock()));
