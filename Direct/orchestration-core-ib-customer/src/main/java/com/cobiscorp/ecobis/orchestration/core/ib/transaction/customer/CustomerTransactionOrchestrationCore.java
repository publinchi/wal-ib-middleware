package com.cobiscorp.ecobis.orchestration.core.ib.transaction.customer;
...
@Component(name = "CustomerTransactionOrchestrationCore", immediate = false)
@Service(value = { ICISSPBaseOrchestration.class, IOrchestrator.class })
@Properties(value = { @Property(name = "service.description", value = "CustomerTransactionOrchestrationCore"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "4.6.1.0"),
		@Property(name = "service.identifier", value = "CustomerTransactionOrchestrationCore") })

public class CustomerTransactionOrchestrationCore extends QueryBaseTemplate {
	ILogger logger = LogFactory.getLogger(CustomerTransactionOrchestrationCore.class);

	@Reference(referenceInterface = ICoreServiceCustomerTransaction.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, bind = "bindCoreService", unbind = "unbindCoreService")
	protected ICoreServiceCustomerTransaction coreServiceCustomerTransaction;
	...
	protected void bindCoreService(ICoreServiceCustomerTransaction service) {
		coreServiceCustomerTransaction = service;
	}
	 ...
	 * @param service
	 */
	protected void unbindCoreService(ICoreServiceCustomerTransaction service) {
		coreServiceCustomerTransaction = null;
	}
	...
	protected IProcedureResponse executeQuery(IProcedureRequest request, Map<String, Object> aBagSPJavaOrchestration) {
		...
		try {
			messageError = "setCustomerTransaction: ERROR EXECUTING SERVICE";
			messageLog = "setCustomerTransaction " + aAddressRequest.getClientCollection().getIdCustomer();
			queryName = "setCustomerTransaction";

			aAddressRequest.setOriginalRequest(request);
			aAddressResponse = coreServiceCustomerTransaction.setCustomerTransaction(aAddressRequest);
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOrginalRequest,
		...
		mapInterfaces.put("coreServiceCustomerTransaction", coreServiceCustomerTransaction);
	...
	private AddressRequest transformAddressRequest(IProcedureRequest aRequest) {
		...
		messageError += aRequest.readValueParam("@i_cod_telefono") == null ? " - @i_cod_telefono can't be null" : "";
		messageError += aRequest.readValueParam("@i_cod_direccion") == null ? " - @i_cod_direccion can't be null" : "";
		messageError += aRequest.readValueParam("@i_cod_email") == null ? " - @i_cod_email can't be null" : "";
		messageError += aRequest.readValueParam("@i_telefono") == null ? " - @i_telefono can't be null" : "";
		messageError += aRequest.readValueParam("@i_barrio") == null ? " - @i_barrio can't be null" : "";
		messageError += aRequest.readValueParam("@i_calle") == null ? " - @i_calle can't be null" : "";
		messageError += aRequest.readValueParam("@i_casa") == null ? " - @i_casa can't be null" : "";
		messageError += aRequest.readValueParam("@i_descripcion") == null ? " - @i_descripcion can't be null" : "";
		messageError += aRequest.readValueParam("@i_email") == null ? " - @i_email can't be null" : "";
		...
		if (aRequest.readValueParam("@i_cod_telefono") != null) {
			aAddress.setPhoneCode(Integer.parseInt(aRequest.readValueParam("@i_cod_telefono")));
		}
		if (aRequest.readValueParam("@i_cod_direccion") != null) {
			aAddress.setAddressCode(Integer.parseInt(aRequest.readValueParam("@i_cod_direccion")));
		}
		if (aRequest.readValueParam("@i_cod_email") != null) {
			aAddress.setEmailCode(Integer.parseInt(aRequest.readValueParam("@i_cod_email")));
		}
		if (aRequest.readValueParam("@i_telefono") != null) {
			aAddress.setPhone(aRequest.readValueParam("@i_telefono"));
		}
		if (aRequest.readValueParam("@i_barrio") != null) {
			aAddress.setNeighborhood(aRequest.readValueParam("@i_barrio"));
		}
		if (aRequest.readValueParam("@i_calle") != null) {
			aAddress.setStreet(aRequest.readValueParam("@i_calle"));
		}
		if (aRequest.readValueParam("@i_casa") != null) {
			aAddress.setHouse(aRequest.readValueParam("@i_casa"));
		}
		if (aRequest.readValueParam("@i_descripcion") != null) {
			aAddress.setBuilding(aRequest.readValueParam("@i_descripcion"));
		}
		if (aRequest.readValueParam("@i_email") != null) {
			aAddress.setEmail(aRequest.readValueParam("@i_email"));
		}

		aAddressRequest.setClientCollection(aClient);
		aAddressRequest.setAddressCollection(aAddress);
	...
	private IProcedureResponse transformProcedureResponse(AddressResponse aAddressResponse,
		...
		if (aAddressResponse.getReturnCode() != 0) {
			...
			metaData.addColumnMetaData(new ResultSetHeaderColumn("name", ICTSTypes.SQLVARCHAR, 200));
			metaData.addColumnMetaData(new ResultSetHeaderColumn("phone", ICTSTypes.SQLVARCHAR, 20));
			metaData.addColumnMetaData(new ResultSetHeaderColumn("email", ICTSTypes.SQLVARCHAR, 254));
			metaData.addColumnMetaData(new ResultSetHeaderColumn("addresInformation", ICTSTypes.SQLVARCHAR, 100));
			...
			if (aAddressResponse.getClientCollection().getCompleteName() != null) {
				row.addRowData(2,
						new ResultSetRowColumnData(false, aAddressResponse.getClientCollection().getCompleteName()));
			} else {
	...
	private boolean IsValidCheckbookResponse(AddressResponse aAddressResponse) {
		...
		messageError += aAddressResponse.getClientCollection().getCompleteName() == null
				? " - CompleteName can't be null" : "";
