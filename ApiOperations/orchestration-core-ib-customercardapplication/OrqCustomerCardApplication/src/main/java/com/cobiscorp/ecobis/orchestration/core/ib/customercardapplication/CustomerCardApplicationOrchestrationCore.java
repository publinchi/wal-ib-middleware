...
public class CustomerCardApplicationOrchestrationCore extends SPJavaOrchestrationBase {
	...
	private IProcedureResponse registerCardApplication(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " Entrando en registerAccount: " + aRequest.readValueParam("@i_org_eje") + " - "+aBagSPJavaOrchestration.containsKey("o_account_id_dock"));
		}
		aBagSPJavaOrchestration.put("ente_mis", aRequest.readValueParam("@i_externalCustomerId"));
		
		IProcedureResponse wAccountsResp = new ProcedureResponseAS();
		
		if(null==aRequest.readValueParam("@i_org_eje")){
			wAccountsResp = getDataClientRegister(aRequest, aBagSPJavaOrchestration);
			if(!wAccountsResp.getResultSetRowColumnData(2, 1, 1).getValue().equals("0"))
				return wAccountsResp;
		}
		
		flowData = aBagSPJavaOrchestration.containsKey("o_account_id_dock") ? aBagSPJavaOrchestration.get("o_account_id_dock").toString() : "X";
		
		logger.logInfo(CLASS_NAME + " ifFindData xdc:" + flowData +"-");
		
		if(flowData.equals("X") || flowData.equals("")){
