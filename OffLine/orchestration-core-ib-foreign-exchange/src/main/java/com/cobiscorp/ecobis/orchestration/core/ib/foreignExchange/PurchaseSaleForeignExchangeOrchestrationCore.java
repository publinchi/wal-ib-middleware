...
public class PurchaseSaleForeignExchangeOrchestrationCore extends OpeningOfflineTemplate {
	...
	protected IProcedureResponse executeOpening(IProcedureRequest request, Map<String, Object> aBagSPJavaOrchestration)
		...
		AccountingParameter parameterDebit = (AccountingParameter) aBagSPJavaOrchestration
				.get("ACCOUNTING_PARAMETER_DEBIT");
		AccountingParameter parameterCredit = (AccountingParameter) aBagSPJavaOrchestration
				.get("ACCOUNTING_PARAMETER_CREDIT");

		// ITO
		/*
		 * if (parameterDebit ==null || parameterCredit==null){ return
		 * Utils.returnException("No existe configuración para la transacción");
		 * }
		 */

		AccountingParameter parameterCommission = (AccountingParameter) aBagSPJavaOrchestration
		...
		if (parameterDebit != null || parameterCredit != null) {
			transferRequest.setCause(parameterDebit.getCause());
			transferRequest.setCauseDes(parameterCredit.getCause());
		}
		/*
		 * transferRequest.setCause(parameterDebit.getCause());
		 * transferRequest.setCauseDes(parameterCredit.getCause());
		...
		if (parameterCommission != null) {
			transferRequest.setCauseComi(parameterCommission.getCause());
			transferRequest.setServiceCost(parameterCommission.getService());
		}
