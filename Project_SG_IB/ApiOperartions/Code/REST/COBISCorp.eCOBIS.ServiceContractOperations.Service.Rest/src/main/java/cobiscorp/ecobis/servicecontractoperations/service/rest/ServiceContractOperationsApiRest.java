...
public class ServiceContractOperationsApiRest {
	...
	public Response  getClientLimits(
		...
		if(!validateMandatory(new Data("externalCustomerId", inRequestGetClientLimits.getExternalCustomerId()), new Data("operation", inRequestGetClientLimits.getOperation()))) {
			LOGGER.logDebug("400 is returned - Required fields are missing");
		...
		LOGGER.logDebug("Ends service execution REST: getClientLimits");
		return Response.ok(outResponseGetClientLimits).build();
	...
	/**
	 * Service to obtain the detail of movements of an existing savings account as new requeriment
	 */
	@POST
	@Path("/apiOperations/accounts/getMovementsDetailV1")
	@Consumes({ "application/json" })
	@Produces({ "application/json" })
	public Response getMovementsDetailV1(
			@NotNull(message = "x-request-id may not be null") @HeaderParam("x-request-id") String xRequestId,
			@NotNull(message = "x-end-user-request-date-time may not be null") @HeaderParam("x-end-user-request-date-time") String xEndUserRequestDateTime,
			@NotNull(message = "x-end-user-ip may not be null") @HeaderParam("x-end-user-ip") String xEndUserIp,
			@NotNull(message = "x-channel may not be null") @HeaderParam("x-channel") String xChannel,
		...
		try {
			outResponseGetMovementsDetail = iServiceContractOperationsApiService.getMovementsDetailV1(xRequestId,
					xEndUserRequestDateTime, xEndUserIp, xChannel, inRequestGetMovementsDetail);
		} catch (CTSRestException e) {
			LOGGER.logError("CTSRestException", e);
			if ("404".equals(e.getMessage())) {
    ...
    */
	...
	return Response.status(500).entity(e.getMessage()).build();
	    ...
	    LOGGER.logDebug("Ends service execution REST: unlockCreditOperation");
	...
	public Response  updateSession(RequestTransferSpi inRequestTransferSpi ){
		LOGGER.logDebug("Start service execution REST: updateSession");
		...
		try {
			outResponseTransferSpi=iServiceContractOperationsApiService.updateSession( inRequestTransferSpi );
		} catch (CTSRestException e) {
			LOGGER.logError("CTSRestException",e);
			if ("404".equals(e.getMessage())) {
				LOGGER.logDebug("404 is returned - No data found");
		...
		LOGGER.logDebug("Ends service execution REST: updateSession");
	...
	public Response  consignmentReverse(
		...
		try {
			outSingleReverseOperationResponse=iServiceContractOperationsApiService.consignmentReverse(xRequestId, xEndUserRequestDateTime, xEndUserIp, xChannel, inReverseOperationRequest );
		} catch (CTSRestException e) {
			if (LOGGER.isErrorEnabled()) LOGGER.logError("CTSRestException", e);
			if ("404".equals(e.getMessage())) {
				if (LOGGER.isDebugEnabled()) LOGGER.logDebug("404 is returned - No data found");
