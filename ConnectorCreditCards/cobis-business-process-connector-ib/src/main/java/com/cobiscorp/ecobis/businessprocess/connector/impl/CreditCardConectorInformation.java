...
import com.cobiscorp.ecobis.ws.client.linkser.DetalleTarjeta;
...
@Component(name = "CreditCardConectorInformation", immediate = false)
@Service({ com.cobiscorp.cobis.csp.services.ICSPExecutorConnector.class })
@Properties({ @Property(name = "service.description", value = "CreditCardConectorInformation"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "1.0.0"),
		@Property(name = "service.identifier", value = "CreditCardConectorInformation") })
public class CreditCardConectorInformation implements ICSPExecutorConnector {
	private static final ILogger logger = LogFactory.getLogger(CreditCardConectorInformation.class);
	...
	public IProcedureResponse transformAndSend(IProcedureRequest anOriginalRequest) {
		...
		detalleTarjeta.setPCodigoLinea(
				fact.createDetalleTarjetaPCodigoLinea(anOriginalRequest.readValueParam("@i_cod_linea")));
		...
		try {
			if (anOriginalRequest.readValueParam("@i_operacion").equals("C")) {
				xmlResponse = iService.detalleTarjeta(detalleTarjeta.getPCodigoLinea().getValue());
				if (logger.isDebugEnabled())
