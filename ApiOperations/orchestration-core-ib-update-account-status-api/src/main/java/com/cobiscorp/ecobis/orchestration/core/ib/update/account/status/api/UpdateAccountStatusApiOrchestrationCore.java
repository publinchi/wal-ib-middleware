...
package com.cobiscorp.ecobis.orchestration.core.ib.update.account.status.api;
 ...
 * 
 */
@Component(name = "UpdateAccountStatusApiOrchestrationCore", immediate = false)
@Service(value = { ICISSPBaseOrchestration.class, IOrchestrator.class })
@Properties(value = { @Property(name = "service.description", value = "UpdateAccountStatusApiOrchestrationCore"),
	@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "4.6.1.0"),
	@Property(name = "service.identifier", value = "UpdateAccountStatusApiOrchestrationCore"),
	@Property(name = "service.spName", value = "cob_procesador..sp_update_account_status_api") })
public class UpdateAccountStatusApiOrchestrationCore extends SPJavaOrchestrationBase {

	private static ILogger logger = LogFactory.getLogger(UpdateAccountStatusApiOrchestrationCore.class);
	private static final String CLASS_NAME = "UpdateAccountStatusApiOrchestrationCore--->";
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOriginalRequest,
		...
		anProcedureResponse = updateAccountStatusVal(anOriginalRequest, aBagSPJavaOrchestration);
	...
	public IProcedureResponse processResponseError(IProcedureResponse anOriginalProcedureRes) {
		if (logger.isInfoEnabled()) {
			logger.logInfo("Starting processResponseError updateAccountStatus--->");
		}
	...
	private IProcedureResponse updateAccountStatusVal(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		
		IProcedureRequest valRequest = new ProcedureRequestAS();
		
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " entering updateAccountStatus request validation...");
		}
		
		valRequest.setSpName("cob_atm..sp_update_account_status_api");
		...
		valRequest.addInputParam("@i_account_status", ICTSTypes.SQLVARCHAR, aRequest.readValueParam("@i_account_status"));
		valRequest.addInputParam("@i_account_number", ICTSTypes.SQLVARCHAR, aRequest.readValueParam("@i_account_number"));
		
		IProcedureResponse valProductsQueryResp = executeCoreBanking(valRequest);

		if (logger.isDebugEnabled()) {
			logger.logDebug("Response updateAccountStatus: " + valProductsQueryResp.getProcedureResponseAsString());
		}

		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " leaving updateAccountStatus request validation...");                                                                              
		}

		
		if (valProductsQueryResp.getResultSets().size() > 2) {
			
			String externalCustomerId = valProductsQueryResp.getResultSetRowColumnData(3, 1, 1).getValue();
		
			aBagSPJavaOrchestration.put("externalCustomerId", externalCustomerId);
		
			return updateAccountStatus(aRequest, aBagSPJavaOrchestration);
		} else {
			return processResponseError(valProductsQueryResp);
		}
	}
	
	private IProcedureResponse updateAccountStatus(IProcedureRequest aRequest,
			Map<String, Object> aBagSPJavaOrchestration) {
		
		IProcedureRequest request = new ProcedureRequestAS();
		
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + "entering updateAccountStatus");
		}
		...
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + "leaving updateAccountStatus");
		}
