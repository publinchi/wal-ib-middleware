 ...
 *
 */
public class Test_ib_physical_sign_validation_third_account_transfer {
	@ClassRule
	...
	/**
	 * Transferencias a cuentas propias con validacion de firmas físicas
	 */
	@Test
	...
	def SsnBranch = "0"
	 ...
	 def Transfer() {
		...
		try{
			...
			wProduct.productId= Integer.valueOf(CTSEnvironment.bvAccCtaCteType)
			wProduct.currencyId= Integer.valueOf(CTSEnvironment.bvAccCtaCteCurrencyId)
			...
			sql("update cobis"+CTSEnvironment.DB_SEPARATOR+"cl_parametro set pa_char = 'S' where pa_nemonico = 'VFFVI'")
