...
class Test_ib_group_change_password {
	@ClassRule
	...
	void TestChangeNewPassword () {
		...
		def rows = sql.executeUpdate("delete cob_bvirtual"+CTSEnvironment.DB_SEPARATOR+"bv_historico_clave where hp_ente="+CTSEnvironment.bvGroupEnte.toString()+" and hp_login = '"+CTSEnvironment.bvGroupLogin+"'")
		...
		try{
			...
			wPassword.oldValue = CTSEnvironment.bvGroupPassword
