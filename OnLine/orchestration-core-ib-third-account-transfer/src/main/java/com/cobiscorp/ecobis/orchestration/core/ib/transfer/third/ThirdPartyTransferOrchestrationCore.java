...
public class ThirdPartyTransferOrchestrationCore extends TransferOnlineTemplate {
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOriginalRequest,
		...
		try {
			...
			aBagSPJavaOrchestration.put(TRANSFER_NAME, "TRANFERENCIA A TERCEROS");
	...
	private IProcedureResponse transformToProcedureResponse(ThirdPartyTransferResponse aThirdPartyTransferResponse,
		...
		if (!aThirdPartyTransferResponse.getSuccess()) {

			aBagSPJavaOrchestration.put(RESPONSE_TRANSACTION,
					Utils.returnException(aThirdPartyTransferResponse.getMessages()));
			...
			if (aThirdPartyTransferResponse != null && !"".equals(aThirdPartyTransferResponse.getAccountStatus())) {
				...
				response.addParam("@o_referencia", ICTSTypes.SYBINT4, 0,
						String.valueOf(aThirdPartyTransferResponse.getReferenceNumber()));
				response.addParam("@o_retorno", ICTSTypes.SYBINT4, 0,
						String.valueOf(aThirdPartyTransferResponse.getReturnValue()));
				response.addParam("@o_condicion", ICTSTypes.SYBINT4, 0,
						String.valueOf(aThirdPartyTransferResponse.getConditionId()));
				response.addParam("@o_autorizacion", ICTSTypes.SYBVARCHAR, 0, "N");
				response.addParam("@o_ssn_branch", ICTSTypes.SYBINT4, 0,
						String.valueOf(aThirdPartyTransferResponse.getBranchSSN()));
