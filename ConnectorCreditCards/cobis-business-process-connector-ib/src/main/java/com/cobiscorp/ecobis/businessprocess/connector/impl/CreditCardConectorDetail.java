...
import com.cobiscorp.ecobis.ws.client.linkser.TarCredCliente;

@Component(name = "CreditCardConectorDetail", immediate = false)
@Service({ com.cobiscorp.cobis.csp.services.ICSPExecutorConnector.class })
@Properties({ @Property(name = "service.description", value = "CreditCardConectorDetail"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "1.0.0"),
		@Property(name = "service.identifier", value = "CreditCardConectorDetail") })
public class CreditCardConectorDetail implements ICSPExecutorConnector {
	private static final ILogger logger = LogFactory.getLogger(CreditCardConectorDetail.class);
	...
	public IProcedureResponse transformAndSend(IProcedureRequest anOriginalRequest) {
		...
		TarCredCliente detalleTarjeta = new TarCredCliente();
		detalleTarjeta.setPCodigoCliente(
				fact.createTarCredClientePCodigoCliente(anOriginalRequest.readValueParam("@i_codigo_cliente")));
		...
		try {
			if (anOriginalRequest.readValueParam("@i_operacion").equals("C")) {
				xmlResponse = iService.tarCredCliente(detalleTarjeta.getPCodigoCliente().getValue());
				if (logger.isDebugEnabled())
