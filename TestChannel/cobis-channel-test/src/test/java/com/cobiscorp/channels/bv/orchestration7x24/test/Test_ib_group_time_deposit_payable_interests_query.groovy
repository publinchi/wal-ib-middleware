 ...
 *
 */
class Test_ib_group_time_deposit_payable_interests_query {
	...
	void testgetTimeDepositPayableInterests(){
		...
		try {
			...
			enquiryRequest.userName = CTSEnvironment.bvGroupLogin;//@i_login
			enquiryRequest.productNumber = CTSEnvironment.bvGroupDpfNumber;//@i_cuenta @i_cta
			enquiryRequest.dateFormatId = CTSEnvironment.bvDateFormat;//@i_formato_fecha
			enquiryRequest.currencyId=CTSEnvironment.bvGroupAccDpfPayableCurrencyid;//@i_mon 
			enquiryRequest.productId=CTSEnvironment.bvAccDpfPayableProductid;//@i_prod
			searchOption.sequential = CTSEnvironment.bvGroupAccDpfPayableSequence;//@i_cuota
			...
			for (var in oPayableInterestResponse) {
				...
				//*******************************************************
				println "**************RESULT GROUP "+ var.payNumber+"*************";
