...
public class AuthorizePurchaseOrchestrationCore extends OfflineApiTemplate {
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOriginalRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if(logger.isDebugEnabled()){
			logger.logDebug("Response ServerStatus: " + serverStatus);
	...
	private IProcedureResponse valTranDataCentralOff(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " Entrando en trnValDataCentralOff ONUS");
		}
		...
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " Saliendo de trnValDataCentralOff ONUS");
		}
	...
	private IProcedureResponse executeOfflinePurchaseCobis(IProcedureRequest anOrgRequest, Map<String, Object> aBagSPJavaOrchestration) {
		if (logger.isInfoEnabled()){
			logger.logInfo(CLASS_NAME + "Ejecutando executeOfflinePurchaseCobis ONUS CORE COBIS" + anOrgRequest);
			logger.logInfo("********** CAUSALES DE TRANSFERENCIA *************"+ aBagSPJavaOrchestration.get("@o_causal"));
		...
		anOriginalRequest.addInputParam("@i_cta", ICTSTypes.SQLVARCHAR, anOrgRequest.readValueParam("@i_account_number"));
		anOriginalRequest.addInputParam("@i_val", ICTSTypes.SQLMONEY4, anOrgRequest.readValueParam("@i_amount"));
		anOriginalRequest.addInputParam("@s_filial", ICTSTypes.SQLINT4, "1");		
		anOriginalRequest.addInputParam("@i_ente", ICTSTypes.SQLINT4, anOrgRequest.readValueParam("@i_external_customer_id"));
		...
		if(response.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
			...
			aBagSPJavaOrchestration.put("@o_ssn_host", "0");
		}
	...
	public IProcedureResponse processResponseApi(IProcedureRequest aRequest, IProcedureResponse anOriginalProcedureRes, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (codeReturn == 0) {
			...
			if(anOriginalProcedureRes.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
				...
				registerMovementsAuthAdditionalData(isOnline,"IDC", Constants.PURCHASE_AT_STORE,(String)aBagSPJavaOrchestration.get("@o_ssn_host"),
			...
			row2.addRowData(2, new ResultSetRowColumnData(false, anOriginalProcedureRes.getResultSetRowColumnData(2, 1, 2)==null?"Service execution error":anOriginalProcedureRes.getResultSetRowColumnData(2, 1, 2).getValue()));
			data2.addRow(row2);
