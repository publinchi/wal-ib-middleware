...
class Test_ib_change_password {
	...
	void TestChangeNewPassword () {
		...
		def rows = sql.executeUpdate("delete cob_bvirtual"+CTSEnvironment.DB_SEPARATOR+"bv_historico_clave where hp_ente="+CTSEnvironment.bvEnte.toString()+" and hp_login = '"+CTSEnvironment.bvLogin+"'")
		...
		try{
			...
			wPassword.oldValue = CTSEnvironment.bvPassword
