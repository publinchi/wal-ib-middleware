...
import com.cobiscorp.ecobis.orchestration.core.ib.charge.template.ChargeOfflineTemplate;
...
@Component(name = "RequestCardOrchestrationCore", immediate = false)
@Service(value = {ICISSPBaseOrchestration.class, IOrchestrator.class})
@Properties(value = {@Property(name = "service.description", value = "RequestCardOrchestrationCore"),
        @Property(name = "service.vendor", value = "COBISCORP"),
        @Property(name = "service.version", value = "4.6.1.0"),
        @Property(name = "service.identifier", value = "RequestCardOrchestrationCore")})
public class RequestCardOrchestrationCore extends ChargeOfflineTemplate {
     ...
     */
    private static ILogger logger = LogFactory.getLogger(RequestCardOrchestrationCore.class);
    ...
    public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOriginalRequest,
        ...
        aBagSPJavaOrchestration.put(CHARGE_NAME, "CARGO POR SOLICITUD DE TARJETA/GASTOS");
    ...
    protected IProcedureResponse executeTransfer(Map<String, Object> aBagSPJavaOrchestration) {
        ...
        try {
            ...
            responseTransfer = this.executeChargeRequestCard(originalRequestClone, aBagSPJavaOrchestration);
            String idMovement = "";
            ...
            if (logger.isDebugEnabled()) {
                logger.logDebug("Response - ID Movimiento: "+ idMovement);
            }
    ...
    public IProcedureResponse executeChargeRequestCard(IProcedureRequest anOriginalRequest,
                                                       Map<String, Object> aBagSPJavaOrchestration) throws CTSServiceException, CTSInfrastructureException {
        if (logger.isDebugEnabled()) {
            logger.logDebug("Inicia executeChargeRequestCard");
        }
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.logDebug("aBagSPJavaOrchestration REQUEST CARD: " + aBagSPJavaOrchestration.toString());
        }

        if (logger.isDebugEnabled()) {
            logger.logDebug("Response accountTransfer:" + response.getProcedureResponseAsString());
            logger.logDebug("Fin executeChargeRequestCard");
        }
    ...
    private IProcedureRequest getRequestTransfer(IProcedureRequest anOriginalRequest,
        ...
        if (logger.isInfoEnabled()) {
            logger.logInfo("Inicia proceso de Cargos por Solicitud de Tarjetas");
        }

        IProcedureRequest requestTransfer = new ProcedureRequestAS();

        requestTransfer.setValueFieldInHeader(ICOBISTS.HEADER_TRN, "1870042");
        ...
        requestTransfer.setSpName("cob_ahorros..sp_cobros_costos");
        requestTransfer.addInputParam("@t_online", ICTSTypes.SQLCHAR, "S");
        requestTransfer.addInputParam("@t_trn", ICTSTypes.SYBINTN, "1870042");
        ...
        if (anOriginalRequest != null && anOriginalRequest.readValueParam("@i_comision") != null) {
            ...
            if (logger.isInfoEnabled())
                logger.logInfo(
                        "Llegada de comisiom  ---> " + anOriginalRequest.readValueParam("@i_comision"));
        ...
        if (logger.isInfoEnabled()) {
            logger.logInfo("Fin proceso de Cargos por Solicitud de Tarjetas");
        }
    ...
    private IProcedureResponse transformToProcedureResponse(IProcedureResponse responseTransfer,
        ...
        if (serverResponse.getOnLine() && responseTransfer.getReturnCode() != 0) {
            ...
            response.addParam("@o_referencia", ICTSTypes.SYBINT4, 0, responseTransfer.readValueParam("@o_referencia"));
            response.setReturnCode(responseTransfer.getReturnCode());
