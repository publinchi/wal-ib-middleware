...
import cobiscorp.ecobis.internetbanking.webapp.commons.dto.TransactionResponse
 ...
 *
 */
class Test_ib_company_checkbook_request_checkbook_query {
	@ClassRule
	...
	void testgetRequestCheckbook() {
		println ' ****** Prueba Regresión testgetRequestCheckbook Persona Natural************* '
		...
		try{
			...
			//TransactionResponse wTransactionResponse= new TransactionResponse();
			
			wCheckbookRequest.transactionId = 1800005; //t_trn
			wCheckbookRequest.operation= 'S';//@i_operacion
			wCheckbookRequest.account=CTSEnvironment.bvAccCtaCteUsdNumber;//@i_cta
			wCheckbookRequest.currency=CTSEnvironment.bvCompanyDpfCurrencyId;//@i_mon
			wCheckbookRequest.typeCheckbook='0122';//@i_tchq
			wCheckbookRequest.numberOfChecks=1;//@i_nchq
			wCheckbookRequest.officeDelivery=1;//@i_ofientr
			wCheckbookRequest.userName=CTSEnvironment.bvCompanyLogin;////@i_login
