...
public class AuthorizeReversalOrchestrationCore extends OfflineApiTemplate {
			...
			if (!flowRty){
				...
				if(anProcedureResponse.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
					...
					if(anProcedureResponse.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
						...
						anProcedureResponse = executeOfflineReversalOnUsCobis(anOriginalRequest, aBagSPJavaOrchestration);
					}
				}				
			}
			else{
	...
	private IProcedureResponse executeOfflineReversalOnUsCobis(IProcedureRequest anOriginalRequest, Map<String, Object> aBagSPJavaOrchestration) {
		if (logger.isInfoEnabled()){
			logger.logInfo(CLASS_NAME + "Ejecutando executeOfflineReversalOnUsCobis CORE COBIS" + anOriginalRequest);
		...
		anOriginalRequest.addInputParam("@i_cta_des", ICTSTypes.SQLVARCHAR, anOriginalRequest.readValueParam("@i_account_number"));
		...
		anOriginalRequest.addInputParam("@i_ente", ICTSTypes.SQLINT4, anOriginalRequest.readValueParam("@i_external_customer_id"));
		...
		anOriginalRequest.addInputParam("@i_origin_uuid", ICTSTypes.SQLVARCHAR, anOriginalRequest.readValueParam("@i_origin_uuid"));
	...
	public IProcedureResponse processResponseApi(IProcedureRequest aRequest, IProcedureResponse anOriginalProcedureRes, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (codeReturn == 0) {
			...
			if(anOriginalProcedureRes.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
				...
				registerMovementsAuthAdditionalData(isOnline,"IDC", Constants.REVERSAL_PHYSICAL,(String)aBagSPJavaOrchestration.get("@o_ssn_host"),
    ...
    public void dataTrn(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
    	 ...
    	 aBagSPJavaOrchestration.put("i_establishmentName",  aRequest.readValueParam("@i_origin_institution_name")); 
