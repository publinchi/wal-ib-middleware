...
class Test_ib_simulator_loan_query {
	...
	@Test
	void testgetSimulationLoan(){
		println ' ****** Prueba Regresión testSimulationLoan ************* '
		def ServiceName = 'getSimulationLoan'
		try {
			...
			println ('------------ **** RESULT **** -----------')
			Loan[] aSimulationLoan = serviceResponseTO.data.get('returnLoan').collect().toArray()
			println "******** Resulset *********"
			...
			for (var in aSimulationLoan) {
				...
				println "productName "+ var.productName;
