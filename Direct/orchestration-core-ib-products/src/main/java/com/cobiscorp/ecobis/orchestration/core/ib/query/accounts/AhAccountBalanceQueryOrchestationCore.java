...
@Component(name = "AhAccountBalanceQueryOrchestationCore", immediate = false)
@Service(value = { ICISSPBaseOrchestration.class, IOrchestrator.class })
@Properties(value = { @Property(name = "service.description", value = "AhAccountBalanceQueryOrchestationCore"), @Property(name = "service.vendor", value = "COBISCORP"),
		@Property(name = "service.version", value = "4.6.1.0"), @Property(name = "service.identifier", value = "AhAccountBalanceQueryOrchestationCore") })
public class AhAccountBalanceQueryOrchestationCore extends QueryBaseTemplate {

	private static final String CLASS_NAME = " >-----> ";
	/**
	 * Instance logger component
	 */
	private static ILogger logger = LogFactory.getLogger(AhAccountBalanceQueryOrchestationCore.class);
	...
	public IProcedureResponse executeTransactionOrchestation(IProcedureRequest anOriginalRequest, Map<String, Object> aBagSPJavaOrchestration) {
		if (logger.isInfoEnabled())
		...
		try {
			BalanceProductResponse wAhBalanceAccount = coreServiceQuery.getSavingAccountBalanceByAccount(balanceProductRequest);

			if (wAhBalanceAccount == null) {
				if (logger.isDebugEnabled())
			...
			if (logger.isDebugEnabled())
				logger.logDebug(CLASS_NAME + "executeTransactionOrchestation:" + wAhBalanceAccount);

			IProcedureResponse wAhBalanceAccountResponse = transformDtoToProcedureResponse(wAhBalanceAccount);
			if ((wAhBalanceAccount != null) && (wAhBalanceAccount.getSuccess()))
	...
	private IProcedureResponse transformDtoToProcedureResponse(BalanceProductResponse balanceProductResponse) {
		...
		if (balanceProductResponse.getSuccess()) {
			...
			metaData.addColumnMetaData(new ResultSetHeaderColumn("clabeAccountNumber", ICTSTypes.SQLVARCHAR, 24));
