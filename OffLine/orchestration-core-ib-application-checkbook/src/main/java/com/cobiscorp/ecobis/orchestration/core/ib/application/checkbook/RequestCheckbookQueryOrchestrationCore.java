package com.cobiscorp.ecobis.orchestration.core.ib.application.checkbook;
...
public class RequestCheckbookQueryOrchestrationCore extends ApplicationOfflineTemplate {
	...
	protected IProcedureResponse executeApplicationCheckbook(IProcedureRequest request,
		...
		if (aBagSPJavaOrchestration.get(ACCOUNTING_PARAMETER) != null) {
			...
			aRequestCheckbookRequest.setCause(accountingParameter.getCause());
		}
		...
		if (aBagSPJavaOrchestration.get(ACCOUNTING_PARAMETER_COMMISSION) != null) {
			...
			aRequestCheckbookRequest.setCauseComi(accountingParameterCommssion.getCause());
			aRequestCheckbookRequest.setServiceCost(accountingParameterCommssion.getService());
		}
		if (logger.isInfoEnabled())
			logger.logInfo("CON CAUSA OBJ: " + aRequestCheckbookRequest.toString());
		try {
			if (logger.isInfoEnabled())
				logger.logInfo("executeQuery");
	...
	private IProcedureResponse transformProcedureResponse(RequestCheckbookResponse aRequestCheckbookResponse,
		...
		if (logger.isInfoEnabled())
			logger.logInfo("JEFF transformProcedureResponse. " + aRequestCheckbookResponse.toString());
		if (logger.isDebugEnabled())
		...
		if (logger.isInfoEnabled())
			logger.logInfo("*******************CODIGO DE RETORNO RequestCheckbook: "
					+ aRequestCheckbookResponse.getReturnCode().toString());
	...
	public NotificationRequest transformNotificationRequest(IProcedureRequest anOriginalRequest,
		...
		if (originProduct.getProductType() == 3)
