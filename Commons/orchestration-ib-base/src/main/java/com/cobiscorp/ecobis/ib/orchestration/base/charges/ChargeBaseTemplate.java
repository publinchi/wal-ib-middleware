package com.cobiscorp.ecobis.ib.orchestration.base.charges;
 ...
 * @since Aug 14, 2014
 */
public abstract class ChargeBaseTemplate extends SPJavaOrchestrationBase {
    ...
    protected static final String RESPONSE_CHARGE = "RESPONSE_CHARGE";
    ...
    private static ILogger logger = LogFactory.getLogger(ChargeBaseTemplate.class);
    ...
    protected IProcedureResponse validateLocalExecution(Map<String, Object> aBagSPJavaOrchestration) {
        ...
        switch (t_trn) {
            case 1870042:
                // COBRO POR SOLICITUD DE CUENTAS DE GASTOS
           ...
           IProcedureResponse responseSigner = new ProcedureResponseAS();
            ...
            if (Utils.flowError("executeTransaction", responseTransfer)) {
                if (responseServer.getOnLine()) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.logDebug(CLASS_NAME + "Response transfer obtained from executing transaction: " + responseTransfer.getProcedureResponseAsString());
                    }

                    if ("S".equals(responseTransfer.readValueFieldInHeader("failProvider")) || getFromReentryExcecution(aBagSPJavaOrchestration)) {
                        if (logger.isInfoEnabled())
   ...
   //     }
    ...
    private boolean validatePreConditions(Map<String, Object> aBagSPJavaOrchestration) {
        ...
        boolean trnValidation = evaluateTrn(anOriginalRequest, 1870042);
