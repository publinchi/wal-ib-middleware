...
import com.cobiscorp.ecobis.ws.client.linkser.WSGETONLINEPAYMENT;

@Component(name = "CreditCardConectorPayment", immediate = false)
@Service({ com.cobiscorp.cobis.csp.services.ICSPExecutorConnector.class })
@Properties({ @Property(name = "service.description", value = "CreditCardConectorPayment"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "1.0.0"),
		@Property(name = "service.identifier", value = "CreditCardConectorPayment") })
public class CreditCardConectorPayment implements ICSPExecutorConnector {
	private static final ILogger logger = LogFactory.getLogger(CreditCardConectorPayment.class);
	...
	public void loadConfiguration(IConfigurationReader configurationReader) {
		...
		properties = configurationReader.getProperties("//config/properties/property");
	...
	public IProcedureResponse processResponseProvider(Map<Object, Object> aBagSPJavaOrchestration, String arg1) {
		...
		// Copio parametros de cabecera desde procedureRequest a
		// procedureResponse
		CSPUtil.copyHeaderFields((IProcedureRequest) aBagSPJavaOrchestration.get("anOriginalRequest"),
				procedureResponse);
	...
	public IProcedureResponse transformAndSend(IProcedureRequest anOriginalRequest) {
		...
		WSGETONLINEPAYMENT pagoTarjeta = new WSGETONLINEPAYMENT();
		pagoTarjeta.setPIdLineaCreditoCobis(
				fact.createWSGETONLINEPAYMENTPIdLineaCreditoCobis(anOriginalRequest.readValueParam("@i_cod_linea")));
		pagoTarjeta.setPIdClienteCobis(
				fact.createWSGETONLINEPAYMENTPIdClienteCobis(anOriginalRequest.readValueParam("@i_cliente")));
		pagoTarjeta.setPIdDocumentoCliente(
				fact.createWSGETONLINEPAYMENTPIdDocumentoCliente(anOriginalRequest.readValueParam("@i_doc_cliente")));
		pagoTarjeta.setPMonto(fact.createWSGETONLINEPAYMENTPMonto(anOriginalRequest.readValueParam("@i_monto")));
		pagoTarjeta.setPIdMoneda(fact.createWSGETONLINEPAYMENTPIdMoneda(anOriginalRequest.readValueParam("@i_moneda")));
		pagoTarjeta
				.setPIdFecha(fact.createWSGETONLINEPAYMENTPIdFecha(anOriginalRequest.readValueParam("@i_fecha_mov")));
		pagoTarjeta
				.setPModPago(fact.createWSGETONLINEPAYMENTPModPago(anOriginalRequest.readValueParam("@i_forma_pago")));
		...
		try {
			if (anOriginalRequest.readValueParam("@i_operacion").equals("C")) {
				xmlResponse = iService.wsgetonlinepayment(pagoTarjeta.getPIdLineaCreditoCobis().getValue(),
						pagoTarjeta.getPIdClienteCobis().getValue(), pagoTarjeta.getPIdDocumentoCliente().getValue(),
						pagoTarjeta.getPMonto().getValue(), pagoTarjeta.getPIdMoneda().getValue(),
						pagoTarjeta.getPIdFecha().getValue(), pagoTarjeta.getPModPago().getValue());
				if (logger.isDebugEnabled())
			...
			iService = null;
			if (logger.isDebugEnabled())
				logger.logDebug("-->> CREDITCARD-CONNECTOR => EXECUTION EXCEPTION - Ejecución al host remoto - ["
						+ anOriginalRequest.readValueParam("@i_tipo_reporte") + "]", e);
			resultados.put("returnCode", -2);
			resultados.put("returnMessage", "WebServiceException - " + e.getMessage());
			return processResponseProvider(resultados, null);
		} finally {
	...
	private boolean getConnection(Map<Object, Object> resultValues) {
		...
		if (iService == null) {
			...
			if (logger.isDebugEnabled()) {
				logger.logDebug("-->> CREDITCARD-CONNECTOR => CONEXION - wsUrl = [" + Constants.URL_STRING + "]");
			}
			try {
				service = new Service1(new URL(Constants.URL_STRING), new QName("http://tempuri.org/", "Service1"));
			} catch (SOAPFaultException e) {
				if (logger.isDebugEnabled())
					logger.logDebug(
							"-->> INFOCRED-CONNECTOR => CONEXION SOAPFaultException - Error al crear la conexión con el Host remoto",
							e);
				resultValues.put("ExceptionMessage", "SOAPFaultException - '" + e.getFault().getFaultCode() + "' - "
			...
			handlerList.add(new CreditCardSecurityHandler(Constants.URL_USER_NAME, Constants.URL_PASSWORD));
