package com.cobiscorp.ecobis.ib.orchestration.base.utils.query;
...
public class ProductPredicate implements Predicate {
	...
	public boolean evaluate(Object arg0) {
		...
		if (logger.isDebugEnabled()){
			...
			logger.logDebug( "Tipo de producto :"+columns[2].getValue()+" - "+this.getProduct().getProductType().toString());
			logger.logDebug( "Moneda :"+columns[3].getValue()+" - "+this.getProduct().getCurrency().getCurrencyId().toString());
		...
		return columns[2].getValue().trim().equals(this.getProduct().getProductType().toString().trim()) &&
			   columns[3].getValue().trim().equals(this.getProduct().getCurrency().getCurrencyId().toString().trim())&&
