...
class Test_ib_company_cashier_check_application {
	@ClassRule
	...
	/**
	 * Metodo que ejecuta la solicitud de cheque de gerencia
	 */
	@Test
	void testCashiersCheckAplication() {
		println ' ****** Prueba Regresión testCashiersCheckAplication Persona Juridica************* '
		...
		try{
			...
			wTransactionRequest.authorizationRequired="N";
			wTransactionRequest.userName=CTSEnvironment.bvCompanyLogin;//@i_login
			wProduct.productNumber=CTSEnvironment.bvCompanyAccCtaAhoNumber;//@i_cta bvAccCtaCteNumber bvAccCtaAhoNumber
			wProduct.productAbbreviation="SB";
			wProduct.currencyId=CTSEnvironment.bvCompanyAccCtaAhoCurrencyId;//@i_mon bvAccCtaCteCurrencyId bvAccCtaAhoCurrencyId
			wProduct.productId=CTSEnvironment.bvAccCtaAhoType;//@i_product bvAccCtaCteType bvAccCtaAhoType
			wManagerCheck.amount=190.53;
			wManagerCheck.beneficiary="PERSONA BENEFICIARIO(PRUEBA REGRESION JURIDICO)";
			wManagerCheck.beneficiaryId="145454521";
			...
			wManagerCheck.authorizedId="2058545454";
			wManagerCheck.authorized="N";
			wManagerCheck.email=CTSEnvironment.bvEmail;
			wManagerCheck.purpose="PROPOSITO PRUEBA REGRESION JURIDICO";
			wContext.entityId=CTSEnvironment.bvCompanyEnteMis;
			...
			println "******** Output ManagerCheckRequest Juridica*********"
