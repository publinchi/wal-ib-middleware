...
public class LoanPaymentOrchestationCore extends PaymentOfflineTemplate {
	...
	public IProcedureRequest anOriginalRequest;
	public AccountingParameterResponse responseAccountingParameters = null;
	...
	// inyecta codigo para pago de prestamo
	...
	protected NotificationRequest transformNotificationRequest(IProcedureRequest anOriginalRequest,
		...
		/*
		 * notification.setId("N1"); notification.setNotificationType("F");
		 */
		...
		originProduct.setProductNumber(anOriginalRequest.readValueParam("@i_cta_des"));
	...
	private PaymentLoanRequest transformToPaymentLoanRequest(IProcedureRequest aRequest) {
		...
		if (aRequest.readValueParam("@i_origen_fondos") != null) {
			apaymentLoanReq.setSourceFunds(aRequest.readValueParam("@i_origen_fondos"));
		}
		if (aRequest.readValueParam("@i_dest_fondos") != null) {
			apaymentLoanReq.setUseFunds(aRequest.readValueParam("@i_dest_fondos"));
		}
		...
		apaymentLoanReq = (PaymentLoanRequest) Utils.setSessionParameters(apaymentLoanReq, aRequest);
	...
	private IProcedureResponse transformToProcedureResponse(PaymentLoanResponse aPaymentLoanResponse,
		...
		IProcedureResponse responseLocalValidation = (IProcedureResponse) aBagSPJavaOrchestration
				.get(RESPONSE_VALIDATE_LOCAL);
		IResultSetData data = new ResultSetData();

		if (serverResponse.getOnLine() && aPaymentLoanResponse.getReturnCode() != 0) {
		...
		if ((!serverResponse.getOnLine() && serverResponse.getOfflineWithBalances())
				|| (serverResponse.getOnLine() && aPaymentLoanResponse.getReturnCode() == 0)) {
			...
			row.addRowData(18, new ResultSetRowColumnData(false, aPaymentLoanResponse.getDateHost()));
		...
		if (aPaymentLoanResponse.getReference() != null) {
			response.addParam("@o_referencia", ICTSTypes.SQLINT4, 0, aPaymentLoanResponse.getReference().toString());
		}
		if (logger.isInfoEnabled())
