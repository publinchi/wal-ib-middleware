...
public class AuthorizeDepositOrchestrationCore extends OfflineApiTemplate {
	...
	private IProcedureResponse valTranDataCentralOff(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " Entrando en valTranDataCentralOff Dep OnUs");
		}
		...
		if (logger.isDebugEnabled()) {
			logger.logDebug("Response Corebanking trnValDataCentralOff Dep OnUs: " + wProductsQueryResp.getProcedureResponseAsString());
		}

		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " Saliendo de trnValDataCentralOff Dep OnUs");
		}
	...
	public IProcedureResponse processResponseApi(IProcedureRequest aRequest, IProcedureResponse anOriginalProcedureRes, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (codeReturn == 0) {
			...
			if(anOriginalProcedureRes.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
				...
				registerMovementsAuthAdditionalData(isOnline,"IDC", Constants.CREDIT_AT_STORE,(String)aBagSPJavaOrchestration.get("@o_ssn_host"),
