 ...
 * @version 1.0.0
 */
@Component(name = "AuthorizePurchaseDockOrchestrationCore", immediate = false)
...
public class AuthorizePurchaseDockOrchestrationCore extends OfflineApiTemplate {
	
	private ILogger logger = (ILogger) this.getLogger();
	private static final String CLASS_NAME = "AuthorizePurchaseDockOrchestrationCore";
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOriginalRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (logger.isDebugEnabled()){logger.logDebug("Begin flow, AuthorizePurchaseDock starts...");}		
		...
		if (serverStatus != null && !serverStatus) {
			...
			if (!flowRty){				
				...
				if(anProcedureResponse.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
					...
					if(anProcedureResponse.getResultSetRowColumnData(2, 1, 1).getValue().equals("0")){
						...
						anProcedureResponse = executeOfflinePurchaseCobis(anOriginalRequest, aBagSPJavaOrchestration);
					}
				}				
			}
			else{
			...
			anProcedureResponse = authorizePurchaseDock(anOriginalRequest, aBagSPJavaOrchestration);
		}
		
		return processResponseApi(anOriginalRequest, anProcedureResponse,aBagSPJavaOrchestration);
	}
	
	private IProcedureResponse authorizePurchaseDock(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " Entrando en authorizePurchaseDock: ");
		}
		
		IProcedureResponse wAuthValDataLocal = new ProcedureResponseAS();
		wAuthValDataLocal = valDataLocalDock(aRequest, aBagSPJavaOrchestration);
		...
		if (logger.isInfoEnabled()) {
			...
			logger.logInfo(CLASS_NAME + " Saliendo de authorizePurchaseDock...");
		}

		return wAuthValDataLocal;
	}
	
	private IProcedureResponse valDataLocalDock(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		request.addInputParam("@i_origin_account_type", ICTSTypes.SQLVARCHAR, aRequest.readValueParam("@i_p_origin_account_type"));
		request.addInputParam("@i_destiny_account_type", ICTSTypes.SQLVARCHAR, aRequest.readValueParam("@i_p_destiny_account_type"));
		...
		request.addInputParam("@i_card_entry_code", ICTSTypes.SQLVARCHAR, aRequest.readValueParam("@i_crd_code"));
		request.addInputParam("@i_pin", ICTSTypes.SQLVARCHAR, aRequest.readValueParam("@i_crd_pin"));
		request.addInputParam("@i_mode", ICTSTypes.SQLVARCHAR, aRequest.readValueParam("@i_crd_mode"));
	...
	private IProcedureResponse trnDataCentral(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		request.addInputParam("@i_cta_deb", ICTSTypes.SQLVARCHAR, aBagSPJavaOrchestration.get("account").toString());
		request.addInputParam("@i_mon_deb", ICTSTypes.SQLINTN, "0");
		request.addInputParam("@i_prod_deb", ICTSTypes.SQLINTN, "4");
		request.addInputParam("@i_val_deb", ICTSTypes.SQLMONEY, aBagSPJavaOrchestration.get("monto").toString()); 
	...
	private IProcedureResponse valTranDataCentralOff(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + " Entrando en trnValDataCentralOff");
		}
		
		request.setSpName("cob_cuentas..sp_val_data_off_purchase");
	...
	private IProcedureResponse executeOfflinePurchaseCobis(IProcedureRequest anOrgRequest, Map<String, Object> aBagSPJavaOrchestration) {
		if (logger.isInfoEnabled()){
		...
		IProcedureRequest anOriginalRequest = anOrgRequest;
	...
	public IProcedureResponse processResponseApi(IProcedureRequest aRequest, IProcedureResponse anOriginalProcedureRes, Map<String, Object> aBagSPJavaOrchestration) {
		...
		if (codeReturn == 0) {		
			...
			if(aBagSPJavaOrchestration.containsKey("s_error")){
				...
				if (logger.isDebugEnabled()){logger.logDebug("Ending flow, processResponse successful...");}
				...
				aBagSPJavaOrchestration.put("i_auth_code",authorizationCode);
				...
				row.addRowData(1, new ResultSetRowColumnData(false, (String) aBagSPJavaOrchestration.get("monto")));
				row.addRowData(2, new ResultSetRowColumnData(false, "0"));
				row.addRowData(3, new ResultSetRowColumnData(false, "0"));
				...
				registerTransactionSuccess("Authorize Purchase Dock", "DOCK", aRequest, 
				...
				String movementType;
                movementType = "VIRTUAL".equals(tipoTarjeta) ? Constants.PURCHASE_ONLINE:Constants.PURCHASE_AT_STORE;
				aBagSPJavaOrchestration.put("i_movement_type",movementType);
				String codAlt = isOnline ? "0" : "1";
                registerMovementsAuthAdditionalData(isOnline,"DOCK", movementType,(String)aBagSPJavaOrchestration.get("@o_ssn_host"),
    ...
    public void dataTrn(IProcedureRequest aRequest, Map<String, Object> aBagSPJavaOrchestration) {
    	 ...
    	 aBagSPJavaOrchestration.put("i_concepto", aRequest.readValueParam("@i_p_type"));
    	 aBagSPJavaOrchestration.put("i_val", aRequest.readValueParam("@i_val_source_value"));
    	 aBagSPJavaOrchestration.put("i_mon", null );
    	 aBagSPJavaOrchestration.put("i_card_id", aRequest.readValueParam("@i_card_id"));
    	 aBagSPJavaOrchestration.put("i_pin", aRequest.readValueParam("@i_crd_pin"));
    	 aBagSPJavaOrchestration.put("i_code", aRequest.readValueParam("@i_crd_code"));
    	 aBagSPJavaOrchestration.put("i_mode", aRequest.readValueParam("@i_crd_mode"));
