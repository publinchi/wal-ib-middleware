 ...
 *
 */
@Component(name = "ProspectiveCustomerOrchestrationCore", immediate = false)
@Service(value = { ICISSPBaseOrchestration.class, IOrchestrator.class })
@Properties(value = { @Property(name = "service.description", value = "ProspectiveCustomerOrchestrationCore"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "4.6.1.0"),
		@Property(name = "service.identifier", value = "ProspectiveCustomerOrchestrationCore") })
public class ProspectiveCustomerOrchestrationCore extends SPJavaOrchestrationBase {

	ILogger logger = LogFactory.getLogger(ProspectiveCustomerOrchestrationCore.class);

	@Reference(referenceInterface = ICoreServiceCustomerProspective.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, bind = "bindCoreService", unbind = "unbindCoreService")
	protected ICoreServiceCustomerProspective coreServiceProspective;

	protected void bindCoreService(ICoreServiceCustomerProspective service) {
		coreServiceProspective = service;
	}

	protected void unbindCoreService(ICoreServiceCustomerProspective service) {
		coreServiceProspective = null;
	}
	...
	private IProcedureResponse transformProcedureResponse(EntityResponse wEntityResponse, String SubType) {
		...
		metaData.addColumnMetaData(new ResultSetHeaderColumn("Bloqueado", ICTSTypes.SQLVARCHAR, 100));

		for (FullEntity wFullEntity : wEntityResponse.getEntityCollection()) {

			if (!IsValidEntityResponse(wFullEntity, SubType))
				return null;
			...
			if (SubType.equals("P")) {
				...
				row.addRowData(11, new ResultSetRowColumnData(false, wFullEntity.getLocked()));
			} else {
				row.addRowData(2, new ResultSetRowColumnData(false, wFullEntity.getCompanyName()));
				row.addRowData(3, new ResultSetRowColumnData(false, wFullEntity.getBusinessName()));
				row.addRowData(4, new ResultSetRowColumnData(false, wFullEntity.getId()));
				row.addRowData(5, new ResultSetRowColumnData(false, wFullEntity.getTypeId()));
				row.addRowData(6, new ResultSetRowColumnData(false, wFullEntity.getOfficial()));
				row.addRowData(7, new ResultSetRowColumnData(false, wFullEntity.getOfficialName()));
				row.addRowData(8, new ResultSetRowColumnData(false, wFullEntity.getLocked()));
			}
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOrginalRequest,
		...
		mapInterfaces.put("coreServiceProspective", coreServiceProspective);
		...
		try {
			...
			messageLog = "getProspectiveCustomer " + wEntityRequest.getSubtipo();
			...
			wEntityResponse = coreServiceProspective.getProspectiveCustomer(wEntityRequest);
	...
	private boolean IsValidEntityResponse(FullEntity wFullEntity, String SubType) {
		String messageError = null;

		messageError = wFullEntity.getId() == null ? " - No. can't be null" : "";
		if (SubType.equals("P")) {
			messageError += wFullEntity.getFirstLast() == null ? " - First Last can't be null" : "";
			messageError += wFullEntity.getFirstName() == null ? " - FirstName can't be null" : "";
		} else {
			messageError += wFullEntity.getCompanyName() == null ? " - Company Name can't be null" : "";
			messageError += wFullEntity.getBusinessName() == null ? " - BusinessName can't be null" : "";
		}

		messageError += wFullEntity.getId() == null ? " - Id can't be null" : "";
