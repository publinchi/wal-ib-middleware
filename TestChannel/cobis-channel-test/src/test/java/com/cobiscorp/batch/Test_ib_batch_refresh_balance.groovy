...
class Test_ib_batch_refresh_balance {
	...
	@Test
	void testBlockedSavingAccountsRefresh() {
		
		def num_registros = 50;
		def siguiente= 0;
		def cuenta= 0;
		...
		while (registros != 0){
			...
			def wRequestJavaOrchCentral = '''<?xml version="1.0" encoding="ISO-8859-1" ?>
					<CTSMessage>
						...
						<Data>
							<ProcedureRequest>
								<SpName>cob_procesador..sp_refresh_bloqueo</SpName>
								<Param name="@t_trn" type="56" io="0" len="7">1875063</Param>
								...
								<Param name="@o_cuenta" type="52" io="1" len="30">[CUENTA]</Param>
					...
					wRequestJavaOrchCentral= wRequestJavaOrchCentral.replace("[CUENTA]", cuenta.toString())
					...
					registros = Integer.valueOf(result.getChildNodes().item(2).getTextContent())
					wXmlResponseSP = wXmlResponseSP.replace("<text>", "text");
	...
	//@Test
	void testBlockedCheckingAccountsRefresh() {
		
		def num_registros = 20;
		def siguiente= 0;
		def cuenta= 0;
		def registros= num_registros;
...
<CTSMessage>
	...
	<Data>
		<ProcedureRequest>
			...
			<Param name="@i_corrida" type="56" io="0" len="1">1</Param>
			<Param name="@i_intento" type="56" io="0" len="1">1</Param>
		    <Param name="@i_operacion" type="56" io="0" len="1">2</Param>
		</ProcedureRequest>
	</Data>
</CTSMessage>
		'''
