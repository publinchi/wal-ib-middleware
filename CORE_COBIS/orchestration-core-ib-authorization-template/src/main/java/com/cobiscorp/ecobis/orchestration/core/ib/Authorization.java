...
@Component(name = "Authorization", immediate = false)
@Service(value = { ICoreServiceAuthorization.class })
@Properties(value = { @Property(name = "service.description", value = "Authorization"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "4.6.1.0"),
		@Property(name = "service.identifier", value = "Authorization") })
public class Authorization extends SPJavaOrchestrationBase implements ICoreServiceAuthorization{

	private static ILogger logger = LogFactory.getLogger(Authorization.class);
	...
	public PendingTransactionResponse changeTransactionStatus(PendingTransactionRequest pendingTransactionRequest)
		...
		if (logger.isInfoEnabled()) {
			logger.logInfo(CLASS_NAME + "INICIANDO SERVICIO changeTransactionStatus");
		}
		
		PendingTransactionResponse pendingTransactionResponse = transformResponse(
				executeChangeTransactionStatus(pendingTransactionRequest));
	...
	private IProcedureResponse executeChangeTransactionStatus(PendingTransactionRequest request) {
		if (logger.isInfoEnabled())
		...
		anOriginalRequest.addInputParam("@s_cliente", ICTSTypes.SYBINT4, request.getEntityId());
		anOriginalRequest.addInputParam("@s_servicio", ICTSTypes.SYBINT2, request.getChannelId());
		anOriginalRequest.addInputParam("@i_operacion", ICTSTypes.SQLVARCHAR, request.getOperation());
		anOriginalRequest.addInputParam("@i_trn_autorizador", ICTSTypes.SQLVARCHAR, request.getTransactionId());
		anOriginalRequest.addInputParam("@i_login", ICTSTypes.SQLVARCHAR, request.getLogin());
		anOriginalRequest.addInputParam("@i_motivo", ICTSTypes.SQLVARCHAR, request.getReason());
