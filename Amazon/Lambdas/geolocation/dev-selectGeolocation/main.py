...
def main_geolocation_local(event, context):
    idPais = 236#mex
    provincias = [
    ...
    try:
        ...
        for coord in coordenadas:
            ...
            if 'pa_id' in coord:
                cliente = coord["pa_id"]
                latitude = coord["pa_latitud"]
                longitude = coord["pa_longitud"]
                ...
                if latitude >= -90 and latitude <= 90 and longitude >= -180 and longitude <= 180: 
                    ...
                    if data is not None and isinstance(data, list) and len(data) > 0 and data[0] is not None and 'Place' in data[0] and 'Country' in data[0]['Place']:  
                        ...
                        try:
                            ...
                            consulta_pais = f"select pa_iso_tif from cob_bvirtual..cl_pais_autorizado, cobis..cl_pais where pa_abreviatura = '{country}' and pa_descripcion = pa_nombre"
                            ...
                            pa_geolocalizacion = pa_geolocalizacion.replace("'", "''")
                            ...
                            if any(provincia["provincia"] == nombre_entidad_federativa for provincia in provincias):
                                ...
                                consulta_geo = (f"""UPDATE cob_bvirtual..bv_pagos 
                                    SET pa_geolocalizacion = '{pa_geolocalizacion}',
                                        pa_id_estado_geo = '{id_estado}',
                                        pa_id_pais_geo = '{id_pais_geo}'
                                    WHERE pa_id = {cliente} """)
                                execute_database(consulta_geo, context)
                            else:
                                pa_geolocalizacion = pa_geolocalizacion.replace('IND','ND')
                                consulta_geo = (f"""UPDATE cob_bvirtual..bv_pagos 
                                    SET pa_geolocalizacion = '{pa_geolocalizacion}',
                                        pa_id_estado_geo = '99999999',
                                        pa_id_pais_geo = '{id_pais_geo}'
                                    WHERE pa_id = {cliente} """)
                                execute_database(consulta_geo, context)
                                # print(consulta_geo)
