package com.cobiscorp.ecobis.ib.orchestration.base.applications;
...
public abstract class ApplicationsBaseTemplate extends SPJavaOrchestrationBase{
	...
	protected IProcedureResponse executeStepsApplicationBase(IProcedureRequest anOriginalRequest, Map<String, Object> aBagSPJavaOrchestration) throws CTSServiceException, CTSInfrastructureException {
		...
		if (Utils.flowError(messageErrorApplication+" --> sendNotification", responseNotification)){ 
			if (logger.isInfoEnabled()) logger.logInfo(CLASS_NAME + messageErrorApplication);
	...
	//WMF
	protected ServerResponse validateServerStatus(IProcedureRequest anOriginalRequest, Map<String, Object> aBagSPJavaOrchestration) throws CTSServiceException, CTSInfrastructureException {
		// TODO Auto-generated method stub
		if (logger.isInfoEnabled()) logger.logInfo(CLASS_NAME + "Iniciando consulta de Estado del Servidor");
			
		if (logger.isDebugEnabled()) logger.logDebug(CLASS_NAME + "Servicio inyectado:" + getCoreService());
			//WMF consultar el estado del servidor
			ServerRequest serverRequest = new ServerRequest();
			if (logger.isDebugEnabled()) logger.logDebug(CLASS_NAME + "WMA: servicio = " + anOriginalRequest.readValueFieldInHeader("servicio"));
			...
			if (logger.isDebugEnabled()) logger.logDebug(CLASS_NAME + "WMF111: responseServer.getReturnCode() = " + responseServer.getReturnCode());
			
			if (logger.isDebugEnabled()) logger.logDebug(CLASS_NAME + "RESPUESTA CORE:" + responseServer);
