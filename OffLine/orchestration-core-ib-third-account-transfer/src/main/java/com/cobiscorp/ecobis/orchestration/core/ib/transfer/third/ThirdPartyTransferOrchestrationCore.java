package com.cobiscorp.ecobis.orchestration.core.ib.transfer.third;
 ...
 *
 */
@Component(name = "ThirdPartyTransferOrchestrationCore", immediate = false)
@Service(value = { ICISSPBaseOrchestration.class, IOrchestrator.class })
@Properties(value = { @Property(name = "service.description", value = "ThirdPartyTransferOrchestrationCore"),
		@Property(name = "service.vendor", value = "COBISCORP"), @Property(name = "service.version", value = "4.6.1.0"),
		@Property(name = "service.identifier", value = "ThirdPartyTransferOrchestrationCore") })
public class ThirdPartyTransferOrchestrationCore extends TransferOfflineTemplate {
	 ...
	 */
	private static ILogger logger = LogFactory.getLogger(ThirdPartyTransferOrchestrationCore.class);
	 ...
	 * Instance plugin to use services other core banking
	 */
	@Reference(referenceInterface = ICoreServiceThirdAccountTransfers.class, cardinality = ReferenceCardinality.OPTIONAL_UNARY, bind = "bindCoreServiceThirdAccountTransfers", unbind = "unbindCoreServiceThirdAccountTransfers")
	private ICoreServiceThirdAccountTransfers coreServiceThirdAccountTransfers;
	 ...
	 * @param service
	 */
	public void bindCoreServiceThirdAccountTransfers(ICoreServiceThirdAccountTransfers service) {
		coreServiceThirdAccountTransfers = service;
	}
	 ...
	 * @param service
	 */
	public void unbindCoreServiceThirdAccountTransfers(ICoreServiceThirdAccountTransfers service) {
		coreServiceThirdAccountTransfers = null;
	}
	...
	public IProcedureResponse executeJavaOrchestration(IProcedureRequest anOriginalRequest,
		...
		mapInterfaces.put("coreServiceThirdAccountTransfers", coreServiceThirdAccountTransfers);

		Utils.validateComponentInstance(mapInterfaces);
		aBagSPJavaOrchestration.put(TRANSFER_NAME, "TRANSFERENCIA A TERCEROS");
		...
		if(response!=null && !response.hasError()) {
			notifyThirdPartyTransfer(originalByNotify, aBagSPJavaOrchestration);
		}
	...
	public IProcedureResponse executeTransfer(Map<String, Object> aBagSPJavaOrchestration) {// throws
		...
		ThirdPartyTransferResponse ThirdPartyTransferResponse = null;
		try {
			ThirdPartyTransferResponse = coreServiceThirdAccountTransfers.executeThirdAccountTransfer(
					UtilsTransfers.transformThirdAccountTransferRequest(aBagSPJavaOrchestration, ORIGINAL_REQUEST,
		...
		return transformToProcedureResponse(ThirdPartyTransferResponse, aBagSPJavaOrchestration);
	}

	private IProcedureResponse transformToProcedureResponse(ThirdPartyTransferResponse aThirdPartyTransferResponse,
			Map<String, Object> aBagSPJavaOrchestration) {
		if (logger.isInfoEnabled())
			logger.logInfo(CLASS_NAME + "transformToProcedureResponse: " + aThirdPartyTransferResponse);
		...
		if (serverResponse.getOnLine() && aThirdPartyTransferResponse.getReturnCode() != 0) {
			// Si estamos en linea y hubo error
			response = Utils.returnException(aThirdPartyTransferResponse.getMessages());
			response.setReturnCode(aThirdPartyTransferResponse.getReturnCode());
		...
		if ((!serverResponse.getOnLine() && serverResponse.getOfflineWithBalances())
				|| (serverResponse.getOnLine() && aThirdPartyTransferResponse.getReturnCode() == 0)) {
			IResultSetData data = new ResultSetData();

			if (aThirdPartyTransferResponse != null && !"".equals(aThirdPartyTransferResponse.getAccountStatus())) {
				...
				row.addRowData(2, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getAvailableBalance().toString()));
				row.addRowData(3, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getAccountingBalance().toString()));
				row.addRowData(4, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getRotateBalance().toString()));
				row.addRowData(5, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getBalance12H().toString()));
				row.addRowData(6, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getBalance24H().toString()));
				row.addRowData(7, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getRemittancesBalance().toString()));
				row.addRowData(8, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getBlockedAmmount().toString()));
				row.addRowData(9, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getBlockedNumber().toString()));
				row.addRowData(10, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getBlockedNumberAmmount().toString()));
				row.addRowData(11, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getOfficeAccount().getId().toString()));
				row.addRowData(12, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getProduct().getProductType().toString()));
				row.addRowData(13,
						new ResultSetRowColumnData(false, aThirdPartyTransferResponse.getBalanceProduct().getState()));
				row.addRowData(14, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getSsnHost().toString()));
				row.addRowData(15, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getSurplusAmmount().toString()));
				row.addRowData(16, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getIdClosed().toString()));
				row.addRowData(17, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getCashBalance().toString()));
				row.addRowData(18, new ResultSetRowColumnData(false, aThirdPartyTransferResponse.getDateHost()));
				row.addRowData(19, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getAvailableBalance().toString()));
				row.addRowData(20, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getAccountingBalance().toString()));
				row.addRowData(21, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getRotateBalance().toString()));
				row.addRowData(22, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getBalance12H().toString()));
				row.addRowData(23, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getBalance24H().toString()));
				row.addRowData(24, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getRemittancesBalance().toString()));
				row.addRowData(25, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getBlockedAmmount().toString()));
				row.addRowData(26, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getBlockedNumber().toString()));
				row.addRowData(27, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getBlockedNumberAmmount().toString()));
				row.addRowData(28, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getOfficeAccount().getId().toString()));
				row.addRowData(29, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getProduct().getProductType().toString()));
				row.addRowData(30, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProductDest().getState()));
				row.addRowData(31, new ResultSetRowColumnData(false,
						aThirdPartyTransferResponse.getBalanceProduct().getSurplusAmmount().toString()));

				row.addRowData(32, new ResultSetRowColumnData(false, aThirdPartyTransferResponse.getName()));
				row.addRowData(33,
						new ResultSetRowColumnData(false, aThirdPartyTransferResponse.getDateLastMovement()));
				row.addRowData(34, new ResultSetRowColumnData(false, aThirdPartyTransferResponse.getAccountStatus()));
		...
		if (logger.isInfoEnabled()) {
			...
			logger.logInfo(CLASS_NAME + "SSN_BRANCH -->" + aThirdPartyTransferResponse.getBranchSSN());
		}
		response.setReturnCode(aThirdPartyTransferResponse.getReturnCode());
		if (logger.isInfoEnabled())
			logger.logInfo(CLASS_NAME + "Respuesta transformToProcedureResponse -->"
	...
	public NotificationRequest transformNotificationRequest(IProcedureRequest anOriginalRequest,
		...
		if (product.getProductType() == 3)
			notification.setId("N8");
		else
			notification.setId("N11");
		...
		if (!Utils.isNull(anOriginalRequest.readParam("@s_ssn"))) //s_ssn_branch
	...
	private void notifyThirdPartyTransfer (IProcedureRequest anOriginalRequest, java.util.Map map) {

		try {

			logger.logInfo("jcos Enviando notificacion cuentas terceros");
			...
			logger.logInfo("jcos Error en la notificacion cuentas terceros");
			logger.logError(xe);
		}
	}
