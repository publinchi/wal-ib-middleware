...
public class SelfAccountTransferOrchestrationCore extends TransferOnlineTemplate {
	...
	private IProcedureResponse transformToProcedureResponse(SelfAccountTransferResponse aSelfAccountTransferResponse,
		...
		if (aSelfAccountTransferResponse.getReturnCode() != 0
				&& aSelfAccountTransferResponse.getReturnCode() != CODE_OFFLINE
				&& aSelfAccountTransferResponse.getReturnCode() != 40003
				&& aSelfAccountTransferResponse.getReturnCode() != 40002) {
			...
			if (aSelfAccountTransferResponse.getReturnCode() != 40002) {
				IResultSetData data = new ResultSetData();

				if (aSelfAccountTransferResponse != null
						&& !"".equals(aSelfAccountTransferResponse.getAccountStatus())) {
					...
					row.addRowData(13, new ResultSetRowColumnData(false,
							aSelfAccountTransferResponse.getBalanceProduct().getState()));
					...
					row.addRowData(28, new ResultSetRowColumnData(false, aSelfAccountTransferResponse
							.getBalanceProductDest().getOfficeAccount().getId().toString()));
					row.addRowData(29, new ResultSetRowColumnData(false, aSelfAccountTransferResponse
							.getBalanceProductDest().getProduct().getProductType().toString()));
					...
					row.addRowData(34,
							new ResultSetRowColumnData(false, aSelfAccountTransferResponse.getAccountStatus()));
