...
public class Test_ib_company_cancel_pending_transaction {
	@ClassRule
	...
	/**
	 * Test to reject authorization of Self Account Transfers
	 */
	@Test
	void testCompanyCancelPendingAuthorization(){
		String ServiceName='CompanyCancelPendingAuthorization'
		
		def IdAuthorization = 3558
		try{
			...
			serviceRequestTO.setServiceId('InternetBanking.WebApp.Authorization.Service.PendingTransaction.CancelPendingTransaction')
			//DTO IN
			Authorizer wAuthorizer=new Authorizer()
			LoginPending wLoginPending=new LoginPending()
			
			wAuthorizer.id = IdAuthorization
			wLoginPending.userName = CTSEnvironment.bvCompanyLogin
			wLoginPending.reason = "REASON: CANCEL PENDING TRANSACTION"
