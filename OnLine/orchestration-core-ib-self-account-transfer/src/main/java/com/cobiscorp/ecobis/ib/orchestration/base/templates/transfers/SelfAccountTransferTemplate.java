...
public abstract class SelfAccountTransferTemplate extends BaseTemplate implements ITransferExecution {
	private static ILogger logger = LogFactory.getLogger(SelfAccountTransferTemplate.class);
	...
	protected IProcedureResponse executeTransaction(IProcedureRequest anOriginalRequest,
		...
		if (SUPPORT_OFFLINE == false) {
			...
			// si estoy en linea ejecuto transferencia no se maneja fuera de
			// linea
			if (serverResponse.getOnLine()) {
				...
				if ((responseTransfer != null) && responseTransfer.getSuccess())
					...
					if (responseTransfer != null) {
						wProcedureRespFinal.setReturnCode(9999);
						wProcedureRespFinal.setText("Error en transferencia ");
						return wProcedureRespFinal;
					} else {
						wProcedureRespFinal.setReturnCode(Integer.parseInt(responseTransfer.getMessage().getCode()));
						wProcedureRespFinal.setText(responseTransfer.getMessage().getDescription());
						return wProcedureRespFinal;
					}
				}
			...
			wProcedureRespFinal.setReturnCode(9999);
			wProcedureRespFinal.setText("Error no se tiene linea");
			return wProcedureRespFinal;
		} else {
			ErrorBlock eb = new ErrorBlock(-1, "Error de Infrestructura");
			wProcedureRespFinal.addResponseBlock(eb);
			wProcedureRespFinal.addFieldInHeader(ICSP.SERVICE_EXECUTION_RESULT, ICOBISTS.HEADER_STRING_TYPE, "1");
			wProcedureRespFinal.setReturnCode(9999);
			wProcedureRespFinal.setText("Error no se maneja fuera de linea validar plugin");
			return wProcedureRespFinal;
		}
	}
	...
	/*
	 * private Boolean getFromReentryExcecution(Map<String, Object>
	 * aBagSPJavaOrchestration) { IProcedureRequest request =
	 * (IProcedureRequest) aBagSPJavaOrchestration.get(ORIGINAL_REQUEST);
	 * logger.logWarning("ERICA valores para  reentry" +
	 * request.readValueFieldInHeader(REENTRY_EXE)); return
	 * ("Y".equals(request.readValueFieldInHeader(REENTRY_EXE))); }
	 */
	...
	protected IProcedureResponse updateLocalExecution(Map<String, Object> aBagSPJavaOrchestration) {
		 ...
		 * originalRequest.addInputParam("@t_rty", ICTSTypes.SQLVARCHAR, "S"); }
		 */
