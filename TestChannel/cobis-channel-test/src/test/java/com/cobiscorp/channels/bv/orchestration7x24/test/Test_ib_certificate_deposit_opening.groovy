 ...
 *
 */
class Test_ib_certificate_deposit_opening {
	@ClassRule
	...
	void testCertificateDepositOpening() {
		println ' ****** Prueba Regresión testCertificateDepositOpening Persona Natural************* '
		...
		try{
			...
			wUser.name=CTSEnvironment.bvLogin;//@i_login;
			wTransactionContextCIB.authenticationRequired="N";
			
			wProduct.productNumber=CTSEnvironment.bvAccCtaAhoNumber;//@i_cta;
			wProduct.currencyId=CTSEnvironment.bvAccCtaAhoCurrencyId;//@i_mon;
			wProduct.productId=CTSEnvironment.bvAccCtaAhoType;//@i_prod;
