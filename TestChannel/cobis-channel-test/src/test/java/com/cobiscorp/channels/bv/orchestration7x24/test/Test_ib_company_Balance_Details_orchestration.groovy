...
class Test_ib_company_Balance_Details_orchestration {	
	...
	void testCompanyGetBalanceDetail(){
		String ServiceName='GetLoanBalance'
		try{
			...
			wEnquiryRequest.productNumber= '10410000041700201'
			...
			for (var in oResponseParameter) {
				...
				println ('------------ RESULTADO -----------')
				println "00 ***  Aditional Data           ---> "+var.result_submit_rpc
				println "01 ***  product number           ---> "+var.productNumber
				println "02 ***  entity name              ---> "+var.entityName
				println "03 ***  operation Type           ---> "+var.operationType
				println "04 ***  initial Amount           ---> "+var.initialAmount
				println "05 ***  monthly Payment Day      ---> "+var.monthlyPaymentDay
				println "06 ***  status                   ---> "+var.status
				println "07 ***  last Payment Date        ---> "+var.lastPaymentDate
				println "08 ***  expiration Date          ---> "+var.expirationDate
				println "09 ***  executive                ---> "+var.executive
				println "10 ***  initial Date             ---> "+var.initialDate
				println "11 ***  arrears Days             ---> "+var.arrearsDays
				println "12 ***  overdue Capital          ---> "+var.overdueCapital
				println "13 ***  overdue Interest         ---> "+var.overdueInterest
				println "14 ***  overdue Arrears Value    ---> "+var.overdueArrearsValue
				println "15 ***  overdue Another Items    ---> "+var.overdueAnotherItems
				println "16 ***  overdue Total            ---> "+var.overdueTotal
				println "17 ***  next Payment Date        ---> "+var.nextPaymentDate
				println "18 ***  next Payment Value       ---> "+var.nextPaymentValue
				println "19 ***  ordinary Interest Rate   ---> "+var.ordinaryInterestRate
				println "20 ***  arrears Interest Rate    ---> "+var.arrearsInterestRate
				println "21 ***  capital Balance Value    ---> "+var.capitalBalance
				println "22 ***  total Balance            ---> "+var.totalBalance
				println "23 ***  original Term            ---> "+var.originalTerm
				println "24 ***  sector                   ---> "+var.sector
				println "25 ***  operation Description    ---> "+var.operationDescription
				println "26 ***  feci                     ---> "+var.feci
			}
		}catch(Exception e){
