...
def call_available_expedients(facade, context): 
    ...
    for event_expedient_file in event_expedient_files:
        ...
        payload = ''
        headers = {
            'WM_CONSUMER.ID': 'f959dee3-23c0-4e2b-b28d-4096962f7b13',
            'WM_SEC.AUTH_SIGNATURE': facade["RESPONSE_JSON_SIGNATURE"]["key"],
            'WM_CONSUMER.INTIMESTAMP': facade["RESPONSE_JSON_SIGNATURE"]["timestamp"]
        }
        conn.request("GET", f"/api-proxy/service/banking/customer-service/v3/customer/kyc-docs?kycStatus=SUCCESS&customer_id={customer_id}", payload, headers)
        ...
        if res.status == 200:
            ...
            for doc in success_documents:
                ...
                if callRequestFile: 
                    ...
                    conn2 = http.client.HTTPSConnection("developer.api.us.stg.walmart.com")
